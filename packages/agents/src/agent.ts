...
> implements IAgent<TContext>
{
  ...
  constructor(config: AgentConfig<TContext>) {
    ...
    // IMPORTANT: Extract toolChoice from modelSettings and do NOT pass it to constructor
    // toolChoice should only be passed per-request, not globally
    ...
    this.aiAgent = new AISDKAgent<Record<string, Tool>>({
      ...
      // Note: maxTurns controls how many steps the agent can take
      // Each step can include tool calls + text generation
      stopWhen: stepCountIs(config.maxTurns || 10),
      temperature: config.temperature,
      // toolChoice removed from constructor - will be passed per-request in stream()
      ...otherModelSettings,
    });
  }
  ...
  stream(
    ...
    // When a user explicitly requests a tool, treat it as a preference rather than
    // forcing the model into tool-call-only mode. This keeps streaming responses
    // working because the model can still produce natural language output.
    let preferredToolInstruction = "";
    let runtimePreferredTool: string | undefined;
    if (toolChoice && toolChoice in resolvedTools) {
      runtimePreferredTool = toolChoice;
      preferredToolInstruction = `<preferred_tool_request>
The user explicitly requested the "${runtimePreferredTool}" tool. Call this tool immediately (using sensible defaults if no arguments are provided), then interpret the results for the user in plain language. After running the tool, ALWAYS provide a natural language response summarizing the findings and recommended actions.
</preferred_tool_request>`;
    }
    ...
    // Extract toolChoice from modelSettings so only configured agents (like triage)
    // are ever forced to call a specific tool.
    const { toolChoice: configuredToolChoice, ...otherSettings } =
      this.modelSettings || {};

    logger.debug(`${this.name} stream() called`, {
      toolChoice,
      preferredToolApplied: Boolean(runtimePreferredTool),
      configuredToolChoice,
      maxSteps,
      hasTools: Object.keys(resolvedTools).length,
    });

    const finalSystemPrompt = preferredToolInstruction
      ? `${preferredToolInstruction}\n\n${systemPrompt}`
      : systemPrompt;

    const additionalOptions: Record<string, unknown> = {
      system: finalSystemPrompt, // Override system prompt per call
      tools: resolvedTools, // Add resolved tools here
      ...otherSettings, // Include other model settings
    };

    if (configuredToolChoice) {
      additionalOptions.toolChoice = configuredToolChoice;
    }
    ...
    // Ensure sufficient steps when toolChoice is set: need at least 2 (tool call + text response)
    const effectiveMaxSteps = toolChoice ? Math.max(maxSteps || 10, 2) : maxSteps;
    if (effectiveMaxSteps) additionalOptions.maxSteps = effectiveMaxSteps;
    if (onStepFinish) additionalOptions.onStepFinish = onStepFinish;

    logger.debug(`${this.name} additionalOptions`, {
      hasToolChoice: !!additionalOptions.toolChoice,
      maxSteps: additionalOptions.maxSteps,
    });

    // Handle simple { messages } format (like working code)
    if ("messages" in options && !("prompt" in options) && options.messages) {
      logger.debug(`${this.name} streaming with messages`, {
        messageCount: options.messages.length,
        firstMessage: options.messages[0]?.role,
        toolChoice: additionalOptions.toolChoice,
        maxSteps: additionalOptions.maxSteps,
      });

      const stream = this.aiAgent.stream({
        messages: options.messages,
        ...additionalOptions,
      });

      logger.debug(`${this.name} stream created successfully`);
      return stream as unknown as AgentStreamResult;
    }
          ...
          try {
            ...
            const filterTextParts = (
              ...
              if (Array.isArray(message.parts)) {
                const allowedTypes =
                  mode === "user" ? ["text"] : ["text"]; // store only text for both
                const filteredParts = stripPartsArray(
                  message.parts,
                  allowedTypes,
                );
                return {
              ...
              if (Array.isArray(message.content)) {
                const allowedTypes =
                  mode === "user" ? ["text"] : ["text"]; // keep only text content
                const filteredContent = stripPartsArray(
            ...
            // Filter out non-text parts (files, reasoning traces, tool metadata)
            const userMsgToSave = filterTextParts(userMsg, "user");
            const assistantMsgToSave = filterTextParts(
              assistantMsg,
              "assistant",
            );
            ...
            await this.saveConversation(
              ...
              JSON.stringify(assistantMsgToSave),
              existingChatForSave,
            );
          } catch (err) {
    ...
    const stream = createUIMessageStream({
      ...
      execute: async ({ writer }) => {
        ...
        const logAgentEvent = (
          ...
          const requestId = runContext.metadata?.requestId || "unknown";
          const prefix = `[agents:${requestId}]`;
          if (payload) {
        ...
        logAgentEvent("orchestrator.start", {
          agent: this.name,
        ...
        try {
          ...
          while (round++ < maxRounds) {
            ...
            // Runtime toolChoice now acts as a preference only (handled via system prompt), but we
            // still validate that the requested tool exists for logging/telemetry.
            let preferredToolForTelemetry = round === 1 ? toolChoice : undefined;
            if (preferredToolForTelemetry) {
              const agentImpl = currentAgent as Agent<any>;
              const agentTools =
                typeof agentImpl.configuredTools === "function"
                  ? agentImpl.configuredTools(executionContext as any)
                  : agentImpl.configuredTools;

              if (!agentTools || !(preferredToolForTelemetry in agentTools)) {
                logger.warn(
                  `Preferred tool ${preferredToolForTelemetry} not found in agent ${currentAgent.name}, ignoring`,
                  {
                    availableTools: Object.keys(agentTools || {}),
                  },
                );
                preferredToolForTelemetry = undefined;
              } else {
                logger.debug(
                  `Validated preferred tool ${preferredToolForTelemetry} exists in ${currentAgent.name}`,
                );
              }
            }
            ...
            logger.debug(`Calling agent.stream for ${currentAgent.name}`, {
              round,
              preferredTool: preferredToolForTelemetry || null,
              maxSteps,
              messageCount: messagesToSend.length,
            });

            logAgentEvent("llm.call.start", {
              agent: currentAgent.name,
              round,
              toolChoice: preferredToolForTelemetry || null,
              messageCount: messagesToSend.length,
              lastMessagePreview:
                typeof lastMessageContent === "string"
                  ? lastMessageContent.slice(0, 160)
                  : Array.isArray(lastMessageContent)
                    ? lastMessageContent
                        .map((part: any) =>
                          typeof part?.text === "string" ? part.text : "",
            ...
            const result = currentAgent.stream({
              ...
              onStepFinish: async (step: unknown) => {
                ...
                logAgentEvent("llm.step.finish", {
                  agent: currentAgent.name,
                  round,
                  step: (step as StepResult<Record<string, Tool>>)?.finishReason,
                });
              },
            } as any);

            logAgentEvent("llm.call.connected", {
              agent: currentAgent.name,
              round,
            });
            ...
            for await (const chunk of uiStream) {
              ...
              if (chunk.type === "tool-output-available") {
                logAgentEvent("tool.call.finish", {
                  agent: currentAgent.name,
                  round,
                  toolName: toolCallNames.get(chunk.toolCallId),
                  hasError: Boolean((chunk as any).error),
                });
              }
            ...
            logAgentEvent("llm.call.finish", {
              agent: currentAgent.name,
              round,
              producedText: textAccumulated.length,
              handoff: Boolean(handoffData),
            });

            // If no text was streamed and no handoff occurred, try a one-shot fallback generation.
            if (!handoffData && textAccumulated.length === 0) {
              try {
                logAgentEvent("llm.fallback.start", {
                  agent: currentAgent.name,
                  round,
                });

                const fallback = await currentAgent.generate({
                  messages: messagesToSend.slice(0, -1),
                  prompt:
                    fallbackPrompt ||
                    "Provide a concise update answering the latest user question based on available data.",
                });
                ...
                if (fallback.text) {
                  ...
                  writer.write({ type: "text-done" } as never);
                  textAccumulated = fallback.text;
                  logAgentEvent("llm.fallback.success", {
                    agent: currentAgent.name,
                    round,
                    chars: fallback.text.length,
                  });
                } else {
                  logAgentEvent("llm.fallback.empty", {
                    agent: currentAgent.name,
                    round,
                  });
                }
              } catch (error) {
                logAgentEvent("llm.fallback.error", {
                  agent: currentAgent.name,
                  round,
          ...
          try {
            logAgentEvent("orchestrator.error", {
              agent: this.name,
              error: error instanceof Error ? error.message : String(error),
            });
          } catch {
...
- Natural next steps, not repetitive`;
   ...
   */
  private sanitizeModelMessage(message: ModelMessage): ModelMessage {
  ...
  private async loadMessagesWithHistory(
    ...
    if (!this.memory?.history?.enabled || !context) {
      ...
      return convertToModelMessages([message]).map((msg) =>
        this.sanitizeModelMessage(msg),
      );
    }
    ...
    try {
      ...
      const historyMessages = convertToModelMessages(previousMessages).map(
        (msg) => this.sanitizeModelMessage(msg),
      );
      ...
      const latestMessages = convertToModelMessages([message]).map((msg) =>
        this.sanitizeModelMessage(msg),
      );
