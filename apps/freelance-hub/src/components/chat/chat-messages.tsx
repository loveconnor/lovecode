    ...
    <>
      {messages.map(({ parts, ...message }, index) => {
        ...
        const errorParts = parts.filter(
          (part): part is ErrorPart => part.type === "error",
        );
        const errorContent = errorParts
          .map((part) => part.error || "Something went wrong.")
          .join("\n");
          ...
          <div key={message.id}>
            ...
            {errorContent && (
              <Message from="assistant">
                <MessageContent
                  variant="flat"
                  className="max-w-[80%] border-destructive text-destructive bg-destructive/10"
                >
                  {errorContent}
                </MessageContent>
              </Message>
