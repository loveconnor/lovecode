...
export function ChatInterface({ user }: ChatInterfaceProps) {
  ...
  const { reset: resetChat } = useChatActions();
  ...
  return (
    <div className="relative flex size-full overflow-hidden min-h-screen">
      <div
        className={cn(
          "fixed left-0 top-0 bottom-0 z-40 w-64 border-r border-border bg-background transition-transform duration-300",
          isHistoryOpen ? "translate-x-0" : "-translate-x-full",
        )}
      >
        <ChatHistory
          onClose={() => setIsHistoryOpen(false)}
          showCloseButton={isHistoryOpen}
        />
      </div>
      {isHistoryOpen && (
        <button
      ...
      <div
        className={cn(
          ...
          isHistoryOpen ? "md:ml-64" : "md:ml-0",
        )}
      >
        {hasMessages ? (
          <>
