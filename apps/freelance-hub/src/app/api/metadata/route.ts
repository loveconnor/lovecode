import { generalAgent } from "@/ai/agents/general";
import { marketIntelAgent } from "@/ai/agents/market-intel";
import { paymentsAgent } from "@/ai/agents/payments";
import { payoutsAgent } from "@/ai/agents/payouts";
import { pipelineAgent } from "@/ai/agents/pipeline";
import { projectsAgent } from "@/ai/agents/projects";
import { studioOpsAgent } from "@/ai/agents/studio-ops";
import { talentAgent } from "@/ai/agents/talent";

// Agent metadata interface
interface AgentMetadata {
  name: string;
  description: string;
  tools: string[];
}

interface ToolMetadata {
  name: string;
  description: string;
  agent: string;
}

interface MetadataResponse {
  agents: AgentMetadata[];
  tools: ToolMetadata[];
}

// Helper to extract tool names from an agent
function getToolNames(agent: {
  configuredTools?: Record<string, unknown>;
}): string[] {
  if (!agent.configuredTools) return [];
  return Object.keys(agent.configuredTools).filter(
    (name) => name !== "handoff_to_agent" && name !== "updateWorkingMemory",
  );
}

// Agent configurations with descriptions
const agentRegistry = [
  {
    agent: pipelineAgent,
    description:
      "Web delivery desk: site bookings, runway, burn, and bench visuals",
  },
  {
    agent: marketIntelAgent,
    description:
      "Market intelligence: freelance web dev rate trends, demand signals, stress tests",
  },
  {
    agent: payoutsAgent,
    description: "Ledger: inflows/outflows on web work, reconciliation spot-checks",
  },
  {
    agent: paymentsAgent,
    description: "Payout operations: invoice creation, status, nudges",
  },
  {
    agent: projectsAgent,
    description: "Web delivery desk: sprint timers, logs, project variance",
  },
  {
    agent: talentAgent,
    description: "Web developer bench: availability, utilization, pairing ideas",
  },
  {
    agent: studioOpsAgent,
    description: "Web program ops: inbox triage, document retrieval, data exports",
  },
  {
    agent: generalAgent,
    description: "Concierge: greetings, routing, quick answers",
  },
];

export async function GET() {
  try {
    const agents: AgentMetadata[] = [];
    const tools: ToolMetadata[] = [];

    for (const { agent, description } of agentRegistry) {
      const toolNames = getToolNames(agent);

      agents.push({
        name: agent.name,
        description,
        tools: toolNames,
      });

      // Add tools with descriptions
      for (const toolName of toolNames) {
        tools.push({
          name: toolName,
          description: `${toolName} tool from ${agent.name} agent`,
          agent: agent.name,
        });
      }
    }

    const response: MetadataResponse = {
      agents,
      tools,
    };

    return new Response(JSON.stringify(response), {
      status: 200,
      headers: {
        "Content-Type": "application/json",
        "Cache-Control": "public, max-age=3600", // Cache for 1 hour
      },
    });
  } catch (error) {
    console.error("Error generating metadata:", error);
    return new Response(
      JSON.stringify({ error: "Failed to generate metadata" }),
      {
        status: 500,
        headers: { "Content-Type": "application/json" },
      },
    );
  }
}
