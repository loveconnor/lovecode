  ...
  execute: async function* (
    ...
    try {
      if (!executionOptions?.experimental_context?.writer) {
        ...
        yield {
          text: `Studio balance for ${from} to ${to}: Total assets ${currency || "USD"} ${data.assets.totalAssets.toLocaleString()} with current ratio ${data.ratios.currentRatio.toFixed(2)}. (Visual board unavailable, showing text summary instead.)`,
        };
        return data;
      }
      ...
      if (!useArtifact) {
        const data = generateBalanceSheet({ from, to, currency, categories });
        yield {
          text: `Studio balance for ${from} → ${to}: total assets ${currency || "USD"} ${data.assets.totalAssets.toLocaleString()} and current ratio ${data.ratios.currentRatio.toFixed(2)}.`,
        };
