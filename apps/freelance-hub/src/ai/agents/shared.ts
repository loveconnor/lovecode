...
- After tool execution, ALWAYS provide a response interpreting the results
...
export const createAgent = (config: AgentConfig<AppContext>) => {
  return new Agent({
    modelSettings: {
      ...
      // Ensure at least 2 turns per agent to allow tool call + response
      ...(config.maxTurns && config.maxTurns < 2 ? {} : {}),
    },
    ...config,
    // Override maxTurns if it's set to 1 (except for triage)
    maxTurns:
      config.name === "triage"
        ? config.maxTurns
        : Math.max(config.maxTurns || 10, 2),
    memory: {
